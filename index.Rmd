...
If you are using **tfdatasets** with the **tfestimators** package, you can pass datasets directly to estimators training functions by using the `input_fn_from_dataset()` function. 
...
Note that we don't use the `dataset_prepare()` or `next_batch()` functions in this example. Rather, these functions are called under the hood by `input_fn_from_dataset()` to provide the `input_fn` interface expected by tfestimators models.
