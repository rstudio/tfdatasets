...
The first thing we need to do after creating the feature_spec is 
to define how you want to treat your variables as categorical or numeric.

You can do this by adding steps to the `spec` object.
...
* `step_categorical_column_with_identity` for categorical variables integerized
...
* `has_type("float32")` to select based on TensorFlow varaible type.
...
Fitting involves evaluatig the full dataset, so If you have provided the vocabulary list and 
your columns are already normalized you can skip the fitting step (TODO).

In our case, we will fit the feature soec, since we didn't specify the vocabulary list
...
We can now prepare our training a testing dataset using:

```{r}
dataset_train <- dataset_train %>% dataset_use_spec(spec_prep)
dataset_test <- dataset_test %>% dataset_use_spec(spec_prep)
```
...
input <- layer_input_from_dataset(dataset_train)

output <- input %>% 
  layer_dense_features(spec_prep$dense_features()) %>% 
...
```{r, warning=FALSE}
history <- model %>% 
  fit(dataset_train, epochs = 15, validation_data = dataset_test)
