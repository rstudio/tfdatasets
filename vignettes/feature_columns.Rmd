...
> This document is an adaptation of the [Feature Columns guide](https://www.tensorflow.org/guide/feature_columns) in the TensorFlow offcial
> website.

This document detais feature columns and how they can be used as inputs to neural
...
following figure shows, all nine functions return either a Categorical-Column or a 
Dense-Column object, except bucketized_column, which inherits from both classes:

![Feature column methods fall into two main categories and one hybrid category.](img/some_constructors.jpg)
...
Even being useful, users may decide to use the `feature_columns` module directly
using `tf$feature_column`.
...
![Dividing year data into four buckets.](img/bucketized_column.jpg)
...
![A categorical identity column mapping. Note that this is a one-hot encoding, not a binary numerical encoding.](img/categorical_column_with_identity.jpg)
...
![Mapping string values to vocabulary columns.](img/categorical_column_with_vocabulary.jpg)
...
You can also place the vocabulary in a separe file that should contain one line 
for each vocabulary element. You can the use:
...
![Representing data with hash buckets.](img/hashed_column.jpg)
...
![Map of Atlanta. Imagine this map divided into 10,000 sections of equal size.](img/Atlanta.jpg)
...
![Representing data in indicator columns.](img/categorical_column_with_identity.jpg)
...
![An embedding column stores categorical data in a lower-dimensional vector than an indicator column. (We just placed random numbers into the embedding vectors; training determines the actual numbers.)](img/embedding_vs_indicator.jpg)
